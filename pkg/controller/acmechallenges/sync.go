...
func (c *controller) Sync(ctx context.Context, chOriginal *cmacme.Challenge) (err error) {
	...
	if !ch.Status.ChallengeAccepted {
		...
		if err != nil {
			c.recorder.Eventf(ch, corev1.EventTypeWarning, reasonAcceptError, "Error accepting challenge: %v", err)
			ch.Status.Reason = err.Error()
			return err
		}
	...
	ch.Status.Reason = fmt.Sprintf("Waiting for %s challenge acceptance: %s", ch.Spec.Type, err)
